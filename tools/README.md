...
| 요일 (UTC `%u`) | 그날 작업 |
|---|---|
| **월·수·금** (1·3·5) | **Task B** · 백로그 집중 소진 (최대 5팀·변형/노이즈 소각 우선) |
...
처리 우선순위: **① 오류 제보(error-inbox, 항상) → ② 추가 제보(inbox, 1회 상한=신규 그룹
3팀 또는 멤버 ~20명; 월·수·금 B슬롯은 5팀) → ③ 그날의 요일 태스크(A/B/C/D, 반드시 수행)**.
추가 제보는 상한까지만 소진하고 나머지는 다음 실행으로 미뤄 **백로그가 커도 요일 태스크가
굶지 않게** 한다(로테이션 보장).

> **백로그 실측(중요).** 개인 제보 수(현재 ~1,300)는 **로마자⇄한글 변형·오타로 과대집계**된
> 값이다. 실질 작업 단위는 **고유 그룹**이며 `ops-metrics.js`의 `backlog.addPendingGroups`
> (`distinctGroups`/`newGroups`/`knownGroups`)로 본다. 개인 수에 겁먹지 말 것.
>
> **변형·노이즈 소각(백로그 최속 감소).** 제보 그룹명이 기존 그룹의 로마자/한글 변형·오타면
> (세븐틴=SEVENTEEN, 에이티즈=ATEEZ, 라이즈=RIIZE, 싸이커스=xikers, 알파드라이브원=ALPHA
> DRIVE ONE) 웹으로 동일 그룹 확인 후 **재추가 말고 `inbox-done`에 정리**. 소속사명(`sm`)·복수
> 그룹 혼합·한두 글자·실존 확인 불가 노이즈도 **반영 말고 `inbox-done` 정리**. 이 소각은 상한과
> 무관하게 최대한 수행(신규 add보다 빠르게 백로그가 준다).

`inbox-done.json`에 이미 있는 key는 처리 완료로 보고 건너뛴다. 처리·소각한 key는
...
7. **UI·데이터를 바꿨으면 push 전 [`qa-checklist.md`](./qa-checklist.md) 를 통과**한다
   (코어 플로우·필터·모바일 오버플로·KO 무결성·정합성). 하나라도 실패면 push 금지.
8. **테스트·장난 제보는 반영 제외**: 이름/그룹이 비었거나 `-` 등 무의미, 명백한 테스트(예:
   `배우`·`솔로` 단독, 실존 확인 불가)는 idols.json에 넣지 말고 그 key만 `inbox-done.json`으로 정리.
